 ...
 */
Dish *newDish(void) { //init dish function
    static int dishId = 1;
...
/**
 * Generate max dish
 * @param A pointer to array of dishes.
 * @param Count of dishes.
 */
 ...
 * @param A pointer to array of dishes.
 * @param Count of dishes.
 * @return Pointer to a dish.
 */
Dish *findDishById (int idOfDish, Dish **dishes){
    Dish* tempDish = {0};
    if(idOfDish <= 0 || idOfDish > 12){
        puts("Error! Wrong Dish id! ");
        return NULL;
    }
    else{
    for(int i = 0; i <= 12; i++){
        if(idOfDish == dishes[i]->menuID){
           tempDish = dishes[i];
            break;
        }
    }
 ...
 */
void printIngredients(const Dish *const dish) {
